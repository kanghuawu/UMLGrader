...
class GradeForm(forms.ModelForm):
	...
	class Meta:
		model = TenantData
		# TF = TenantFields.objects.filter(tenant_id=thisuser).order_by('field_column')
	...
	# 	self.fields = [f.name for f in TenantData._meta.get_fields()][3:TF_length+3]
	# 	self.labels = dict(zip(fields, TF_name))
